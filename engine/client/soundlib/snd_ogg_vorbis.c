...
qboolean Sound_LoadOggVorbis( const char *name, const byte *buffer, fs_offset_t filesize )
{
	...
	while(( ret = ov_read( &vorbisFile, (char *)sound.wav + written, sound.size - written, XASH_BIG_ENDIAN ? 1 : 0, sound.width, 1, &section )) != 0 )
	{
		if( ret < 0 )
		{
			Con_DPrintf( S_ERROR "%s: failed to load (%s): %s\n", __func__, file.name, Vorbis_GetErrorDesc( ret ));
...
int Stream_ReadOggVorbis( stream_t *stream, int needBytes, void *buffer )
{
	...
	while( 1 )
	{
		...
		if( !stream->buffsize )
		{
			stream->pos = ov_read( &ctx->vf, (char *)stream->temp, OUTBUF_SIZE, XASH_BIG_ENDIAN ? 1 : 0, stream->width, 1, &section );
			if( stream->pos == 0 )
			{
				break; // end of file
			}
