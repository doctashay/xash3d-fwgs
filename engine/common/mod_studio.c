...
void *R_StudioGetAnim( studiohdr_t *m_pStudioHeader, model_t *m_pSubModel, mstudioseqdesc_t *pseqdesc )
{
	...
	if( !Mod_CacheCheck(( cache_user_t *)&( paSequences[pseqdesc->seqgroup] )))
	{
		...
		byte *seqgroup = (byte *)paSequences[pseqdesc->seqgroup].data;

		for( int i = 0; i < m_pStudioHeader->numseq; i++ )
		{
			if( pseqdesc2[i].seqgroup == pseqdesc->seqgroup )
				Mod_SwapStudioSeqGroupAnims( m_pStudioHeader, &pseqdesc2[i], seqgroup );
		}
	}
...
static qboolean Mod_IsCounterStrikeGame( void )
{
	return GI && ( !Q_stricmp( GI->gamefolder, "cstrike" ) || !Q_stricmp( GI->gamefolder, "czero" ) );
}

static qboolean Mod_StudioBoundsAreSet( const vec3_t mins, const vec3_t maxs )
{
	return !VectorCompare( vec3_origin, mins ) || !VectorCompare( vec3_origin, maxs );
}
