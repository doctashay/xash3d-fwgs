...
void R_StudioCalcBones( int frame, float s, const mstudiobone_t *pbone, const mstudioanim_t *panim, const float *adj, vec3_t pos, vec4_t q )
{
	...
	for( i = 0; i < max; i++ )
	{
		...
		{
			v1[i] = (int16_t)LittleShort( panimvalue[j + 1].value );

			if( panimvalue->num.valid > j + 1 )
				v2[i] = (int16_t)LittleShort( panimvalue[j + 2].value );
			else if( panimvalue->num.total > j + 1 )
				v2[i] = v1[i];
			else
				v2[i] = (int16_t)LittleShort( panimvalue[panimvalue->num.valid + 2].value );
		}
		else
		{
			v1[i] = (int16_t)LittleShort( panimvalue[panimvalue->num.valid].value );
